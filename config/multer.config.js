...
// ============================================================
// � CLOUDINARY STORAGE FOR CANDIDATE FILES
// ============================================================
...
const candidateStorage = new CloudinaryStorage({
  ...
  params: async (req, file) => {
    // Determine Cloudinary subfolder based on field name
    let folder = 'ictforum/candidates/others';
    ...
    return {
      ...
      public_id: `${file.fieldname}-${name}-${uniqueSuffix}`,
      resource_type: file.fieldname === 'manifestoBrochure' && file.mimetype === 'application/pdf' ? 'raw' : 'image',
    };
  },
});
...
// For multi-field candidate uploads → req.files
...
module.exports = {
  ...
  candidateUpload, // candidate multi-field upload
  postUploader,  // raw post multer instance (optional)
  memberUploader, // raw member multer instance (optional)
  candidateUploader // raw candidate multer instance (optional)
};
